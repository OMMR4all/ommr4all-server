...
class OMRTrainer(SymbolDetectionTrainerBase):
    ...
    def run(self, model_for_book: Optional[DatabaseBook] = None, callback: Optional[SymbolDetectionTrainerCallback] = None):
        train_dataset = self.train_dataset.to_music_line_calamari_dataset(train=True)
        val_dataset = self.validation_dataset.to_music_line_calamari_dataset(train=True)
        ...
        params.model.network.channels = self.params.calamari_params.channels
