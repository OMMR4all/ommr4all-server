from enum import IntEnum
from rest_framework.response import Response
from mashumaro import DataClassDictMixin
from dataclasses import dataclass


class ErrorCodes(IntEnum):
    # Global
    UNKNOWN_ERROR = 1000
    INVALID_CREDENTIALS = 1001

    CONNECTION_TO_SERVER_TIMED_OUT = 10001
    SERVER_DATABASE_UNAVAILABLE = 10002

    # Global permissions
    GLOBAL_INSUFFICIENT_RIGHTS = 21003

    # book style related
    BOOK_STYLE_INVALID_REQUEST = 23001
    BOOK_STYLE_INVALID_NAME = 23002
    BOOK_STYLE_EXISTS = 23003

    # symbol class related
    SYMBOL_CLASS_INVALID_REQUEST = 24001
    SYMBOL_CLASS_INVALID_NAME = 24002
    SYMBOL_CLASS_EXISTS = 24003
    SYMBOL_CLASS_DIGIT_SHORTCUT_TAKEN = 24004
    SYMBOL_CLASS_GLYPH_REQUIRED = 24005

    # Book related
    BOOK_EXISTS = 41001
    BOOK_INVALID_NAME = 41002
    BOOK_INSUFFICIENT_RIGHTS = 41003
    BOOK_OPERATIONS_LOCKED = 41004

    BOOK_PAGE_UPLOAD_FAILED_PAYLOAD_TOO_LARGE = 41010
    BOOK_IMPORT_FAILED_INVALID_STRUCTURE = 41011
    BOOK_IMPORT_FAILED_BOOK_EXISTS = 41012

    BOOK_PAGES_RENAME_REQUIRE_UNIQUE_SOURCES = 41030
    BOOK_PAGES_RENAME_REQUIRE_UNIQUE_TARGETS = 41031
    BOOK_PAGES_RENAME_TARGET_EXISTS = 41032

    BOOK_ASSIGNMENT_UNKNOWN_USER = 41040
    BOOK_ASSIGNMENT_UNKNOWN_PAGE = 41041
    BOOK_ASSIGNMENT_NOT_FOUND = 41042
    BOOK_ASSIGNMENT_INVALID_COUNT = 41043
    BOOK_ASSIGNMENT_INSUFFICIENT_PAGES = 41044
    BOOK_ASSIGNMENT_INVALID_RANGE = 41045


    # Page related
    PAGE_EXISTS = 44001
    PAGE_INVALID_NAME = 44002
    PAGE_NOT_LOCKED = 44003

    # Page progress
    PAGE_PROGRESS_VERIFICATION_REQUIRES_ALL_PROGRESS_LOCKS = 44101
    PAGE_VERIFICATION_REQUIRED = 44102
    PAGE_IS_VERIFIED = 44103

    # Operation related
    OPERATION_INVALID_GET = 50001
    OPERATION_UNKNOWN_SERVER_ERROR = 50002

    # Task related
    OPERATION_TASK_NOT_FOUND = 51001
    OPERATION_TASK_NO_MODEL = 51002
    OPERATION_TASK_INVALID_WORKER_RESOURCE = 51003
    OPERATION_TASK_WORKER_RESOURCE_UNAVAILABLE = 51004

    # Task training related
    OPERATION_TASK_TRAIN_EMPTY_DATASET = 52001
    OPERATION_TASK_INVALID_TRAINER_PARAMS = 52002

    # Model related
    MODEL_NOT_FOUND = 55000

    # Document related
    DOCUMENT_NOT_FOUND = 60000
    DOCUMENT_UNKNOWN_EXPORT_FORMAT = 60001

    # Monodi related
    MONODI_LOGIN_REQUIRED = 65000
    NO_MATCHING_CREDENTIALS_FOUND = 66000
    ERROR_ON_IMPORTING_DOCUMENTS = 67000
    ERROR_ON_UPDATING_DOCUMENT = 68000


@dataclass
class APIError(DataClassDictMixin):
    status: int
    developerMessage: str
    userMessage: str
    errorCode: ErrorCodes

    def response(self):
        return Response(self.to_dict(), status=self.status)


class DatabaseUnavailableAPIError(APIError):
    """The database could not be read or written, even after reconnecting.

    Distinct from 'no lock' on purpose: an unavailable database must never be reported as
    'nobody holds the lock', and the client must not silently drop out of edit mode."""
    def __init__(self, detail: str):
        from rest_framework import status
        super().__init__(status.HTTP_503_SERVICE_UNAVAILABLE,
                         'Database error: {}'.format(detail),
                         'The server database is temporarily unavailable. '
                         'Your work is not lost, please retry in a moment.',
                         ErrorCodes.SERVER_DATABASE_UNAVAILABLE,
                         )


class PageNotLockedAPIError(APIError):
    def __init__(self, status):
        super().__init__(status, 'Page not locked by user. Access denied.',
                         'Access denied. You did not request access to the page',
                         ErrorCodes.PAGE_NOT_LOCKED
                         )

