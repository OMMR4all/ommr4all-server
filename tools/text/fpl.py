...
for i in data:
    ...
    images = filted_images
    dirs = [f for f in os.listdir(os.path.join(path, i[0])) if os.path.isdir(os.path.join(path, i[0], f))]
    print(dirs)
    for d in dirs:
        print(d)
        si= d.split("_")[-1]
        model = os.path.join(path, i[0], d + "/model_best.pth")
        for img_path in images:
            ...
            with torch.no_grad():
                ...
                with open(Path(img_path).with_suffix(f".lstm_f{si}.txt"), "w") as f:
