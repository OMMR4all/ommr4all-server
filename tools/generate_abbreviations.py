    ...
    with open(csv_path) as csvfile:
        ...
        for row in csv_reader:
            if first_line:
                ...
                continue
            chant = row[18]
            chant=chant.replace("-", "").replace("*","").replace("(","").replace(")", "").replace(".", "").strip().lower().replace("{","").replace("}","").replace("[","").replace("]","").replace(",","").replace("#","").replace("|","").replace(":","")
            chant_manuscript = row[19]
            chant_manuscript=chant_manuscript.replace("-", "").replace("*","").replace("(","").replace(")", "").replace(".", "").strip().lower().replace("{","").replace("}","").replace("[","").replace("]","").replace(",","").replace("#","").replace("|","").replace(":","")
